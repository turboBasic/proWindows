...
<# 
	...
	.EXAMPLE
		ExpandWindowsImage `
			-mediaPath w:\ISOs\ws2016.iso -wimPath = '\sources\install.wim' -wimImageIndex 1 `
			-vhd b:\test\wadk.vhdx -partitionStyle MBR `
			-windowsOptionalFeature  '...', '...' -sourcePath = '\sources\sxs' `
...
#>
    ...
    PROCESS {
		...
		WriteVerbose $expandWindowsImage
